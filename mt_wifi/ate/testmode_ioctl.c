...
#ifdef MT7626
	ChipId = pAd->ChipID;
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: ChipId = %x\n", __func__, ChipId));
#else
	RTMP_IO_READ32(pAd->hdev_ctrl, TOP_HCR, &ChipId);
#endif
...
#ifdef MT7626
		HW_IO_READ32(pAd->hdev_ctrl, 0x700c, &RestoreValue);
		RemapBase = Offset & 0xffff0000;
		RemapOffset = Offset & 0xffff;
		HW_IO_WRITE32(pAd->hdev_ctrl, 0x700c, RemapBase);
		HW_IO_READ32(pAd->hdev_ctrl, 0x90000 + RemapOffset, &Value);
		HW_IO_WRITE32(pAd->hdev_ctrl, 0x700c, RestoreValue);
#else
		HW_IO_READ32(pAd->hdev_ctrl, MCU_PCIE_REMAP_2, &RestoreValue);
		RemapBase = GET_REMAP_2_BASE(Offset) << 19;
		RemapOffset = GET_REMAP_2_OFFSET(Offset);
		HW_IO_WRITE32(pAd->hdev_ctrl, MCU_PCIE_REMAP_2, RemapBase);
		HW_IO_READ32(pAd->hdev_ctrl, 0x80000 + RemapOffset, &Value);
		HW_IO_WRITE32(pAd->hdev_ctrl, MCU_PCIE_REMAP_2, RestoreValue);
#endif
	} else
		HW_IO_READ32(pAd->hdev_ctrl, Offset, &Value);
	...
	struct _HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	if (!IsFound) {
		...
		RemapOffset = Offset & 0xffff;
		HW_IO_WRITE32(pAd->hdev_ctrl, 0x700c, RemapBase);
		HW_IO_WRITE32(pAd->hdev_ctrl, 0x90000 + RemapOffset, Value);
		HW_IO_WRITE32(pAd->hdev_ctrl, 0x700c, RestoreValue);
#else
		HW_IO_READ32(pAd->hdev_ctrl, MCU_PCIE_REMAP_2, &RestoreValue);
		RemapBase = GET_REMAP_2_BASE(Offset) << 19;
		RemapOffset = GET_REMAP_2_OFFSET(Offset);
		HW_IO_WRITE32(pAd->hdev_ctrl, MCU_PCIE_REMAP_2, RemapBase);
		HW_IO_WRITE32(pAd->hdev_ctrl, 0x80000 + RemapOffset, Value);
		HW_IO_WRITE32(pAd->hdev_ctrl, MCU_PCIE_REMAP_2, RestoreValue);
#endif
	} else
		HW_IO_WRITE32(pAd->hdev_ctrl, Offset, Value);
	...
	struct _HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	if (Offset + Len <= size)
		...
		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
				 ("%s: exceed EEPROM size (offset:%d, size:%d)\n", __func__, Offset+Len, size));
		Len = 0;
		Ret = -1;
	}
	...
	struct _HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			 ("%s: Version:%d, DieIndex:%d\n",
			 __func__, Version, DieIndex));
	...
	if (ATEOp->EfuseGetFreeBlock) {
		if (Version == 1) {
			...
			MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
					("%s: ucGetFreeBlock:%d, ucGetTotalBlock:%d\n",
					__func__, EventEfuseFreeBlock.ucFreeBlockNum, EventEfuseFreeBlock.ucTotalBlockNum));
	...
	struct _HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	if (ATEOp->GetCfgOnOff) {
		...
		if ((IS_MT7615(pAd) || IS_MT7622(pAd) || IS_P18(pAd) || IS_MT7663(pAd) || IS_AXE(pAd) || IS_MT7626(pAd)) && Type != 2)
#endif
		{
...
VOID HQA_GetThermalValue_CB(struct cmd_msg *msg, char *Data, UINT16 Len)
{
	...
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			 ("%s(): value: 0x%x\n", __func__, prEventExtCmdResult->u4SensorResult));
	...
	struct _HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s band_idx:%d\n", __func__, control_band_idx));
	memcpy((UINT8 *)&Value, (UINT8 *)&HqaCmdFrame->Data, 4);
	Value = PKTL_TRAN_TO_HOST(Value);
	TESTMODE_SET_PARAM(ATECtrl, control_band_idx, Stbc, Value);
	...
	struct _HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	memcpy((PUCHAR)&Value, (PUCHAR)&HqaCmdFrame->Data, 4);
	Value = PKTL_TRAN_TO_HOST(Value);
	TESTMODE_SET_PARAM(ATECtrl, control_band_idx, Sgi, Value);
	...
	struct _HQA_CMD_FRAME *HqaCmdFrame)
{
...
#if defined(MT7626)
	if (control_band_idx == TESTMODE_BAND1 && IS_MT7626(pAd)) {
		value = MtAsicGetRxStat(pAd, HQA_RX_STAT_RSSI_BAND1);
		HQA_ParseRxRssiCR(pAd, &HqaRxStat, HQA_RX_STAT_RSSI_BAND1, value, Ch_Band_1);
	} else
#endif
	{
	...
	if (control_band_idx == TESTMODE_BAND1 && IS_MT7626(pAd)) {
		...
		HQA_ParseRxRssiCR(pAd, &HqaRxStat, HQA_RX_STAT_RSSI_RX23_BAND1, value, Ch_Band_1);
		...
		HQA_ParseRxRssiCR(pAd, &HqaRxStat, HQA_RX_STAT_RSSI_RX23, value, Ch_Band_0);
	...
	value = MtAsicGetRxStat(pAd, HQA_RX_STAT_PHY_FCSERRCNT_BAND1);
	HqaRxStat.phy_rx_fcs_err_cnt_ofdm_band1 = PKTL_TRAN_TO_HOST(value >> 16);
	HqaRxStat.phy_rx_fcs_err_cnt_cck_band1 = PKTL_TRAN_TO_HOST(value & 0xFFFF);
	...
	for (i = 0; i < 4; i++) {
		if (IS_MT7626(pAd)) {
			/* FAGC_RSSI_IB0 for band0, FAGC_RSSI_IB1 for band1, others clean */
			if (control_band_idx == TESTMODE_BAND0 && i == 0)
				value = MtATERSSIOffset(pAd, ATECtrl->rx_stat.FAGC_RSSI_IB[i], FAGC_Path_0, Ch_Band_0);
			else if (control_band_idx == TESTMODE_BAND1 && i == 1)
				value = MtATERSSIOffset(pAd, ATECtrl->rx_stat.FAGC_RSSI_IB[i], FAGC_Path_1, Ch_Band_1);
			else
				value = 0;
		} else {
			if (i < 2)
		...
		HqaRxStat.FAGC_IB_RSSSI[i] = PKTL_TRAN_TO_HOST(value);
	...
	for (i = 0; i < 4; i++) {
		if (IS_MT7626(pAd)) {
			/* FAGC_RSSI_IB0 for band0, FAGC_RSSI_IB1 for band1, others clean */
			if (control_band_idx == TESTMODE_BAND0 && i == 0)
				value = MtATERSSIOffset(pAd, ATECtrl->rx_stat.FAGC_RSSI_WB[i], FAGC_Path_0, Ch_Band_0);
			else if (control_band_idx == TESTMODE_BAND1 && i == 1)
				value = MtATERSSIOffset(pAd, ATECtrl->rx_stat.FAGC_RSSI_WB[i], FAGC_Path_1, Ch_Band_1);
			else
				value = 0;
		} else {
			if (i < 2)
		...
		HqaRxStat.FAGC_WB_RSSSI[i] = PKTL_TRAN_TO_HOST(value);
	}
...
#if defined(MT7615) || defined(MT7622) || defined(MT7626)
...
#endif /* defined(MT7615) || defined(MT7622) || defined(MT7626) */
	{
		for (i = 0; i < 4; i++)
			MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_INFO,
				("Report[0]: RCPI[%d]:%x\n",
	...
	struct _HQA_CMD_FRAME *HqaCmdFrame)
{
...
#if !defined(COMPOS_TESTMODE_WIN)
	UCHAR *data = HqaCmdFrame->Data;
	UINT32 value = 0, version = 0, throughold, iter;
	CHAR TMR_Value[8];
	...
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			 ("%s: TMR setting: %u, TMR version: %u TMR throughold: %d TMR iter:%d\n",
			  __func__, value, version, throughold, iter));
	...
	if (log_cb->is_dumping) {
		if (log_type == ATE_LOG_RE_CAL) {
			if (log_ctrl == ATE_LOG_ON)
				...
				MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF,
						("%s: Shouldn't happen ! log_ctrl:%d\n", __func__, log_ctrl));
				ATECtrl->reCalInDumpSts = DISABLE_RECAL_IN_DUMP;
			}
			MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF,
					("%s: Recal Log dumping, log_ctrl:%d, log_type:%d, reCalInDumpSts:%d\n",
					__func__, log_ctrl, log_type, ATECtrl->reCalInDumpSts));
			return TRUE;
		} else {
...
static INT32 HQA_BFQdRead(PRTMP_ADAPTER pAd, RTMP_IOCTL_INPUT_STRUCT *WRQ, struct _HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	PKTLA_DUMP(DBG_LVL_TRACE, 14, out);
	NdisMoveMemory((PUCHAR)&HqaCmdFrame->Data + 2, (PUCHAR)out, ate_ctrl->txbf_info_len);
	ate_ctrl->Mode &= ~fATE_IN_BF;
	os_free_mem(ate_ctrl->txbf_info);
	ate_ctrl->txbf_info = NULL;
...
static INT32 HQA_MUGetQD(PRTMP_ADAPTER pAd, RTMP_IOCTL_INPUT_STRUCT *WRQ, struct _HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	PKTLA_DUMP(DBG_LVL_TRACE, sizeof(qd) / sizeof(int), &qd);
...
static INT32 HQA_CapWiFiSpectrum(PRTMP_ADAPTER pAd, RTMP_IOCTL_INPUT_STRUCT *WRQ, struct _HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	UINT32 PhyIdx, SourceAddrLSB = 0, SourceAddrMSB = 0;
		...
		if (ATEOp->SetICapStart) {
			{
				...
				MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("PhyIdx:%d\n", PhyIdx));
...
#define MAX_RECAL_DUMP_SIZE 100
	...
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("[RECAL DUMP START][HQA_GetDumpRecal]\n"));
	pAd->fgDumpStart = 1;
	...
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("idx: %d\n", idx));
	...
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("[After dumping] idx: %d, end: %d\n", idx, end));

	/* RECAL DUMP END */
	/* Count = Total number of 4 bytes RECAL values divided by 3 */
	count_total = count_total / 3;
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("[HQA_GetDumpRecal]: Total Count As a Group of three = %d\n", count_total));
	if ((idx == log_cb->idx) && (pAd->fgDumpStart)) {
		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("[RECAL DUMP END]\n"));
		pAd->fgDumpStart = 0;
	}
	...
	if (count_total > 0) {
		...
		if (ATECtrl->reCalInDumpSts == DISABLE_RECAL_IN_DUMP) {
			struct _ATE_OPERATION *ATEOp = ATECtrl->ATEOp;
			MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF,
					 ("%s:LogOnOff, log_type:%d, log_ctrl:%d, log_size:%d\n",
					  __func__, ATECtrl->logStsInDump[0], ATECtrl->logStsInDump[1], ATECtrl->logStsInDump[2]));
			ATEOp->LogOnOff(pAd, ATECtrl->logStsInDump[0], ATECtrl->logStsInDump[1], ATECtrl->logStsInDump[2]);
...
#define MAX_RDD_DUMP_SIZE 100
	...
	log_cb->is_dumping = TRUE;
	OS_SPIN_UNLOCK(&log_cb->lock);
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("[RDD DUMP START][HQA_GetDumpRDD]\n"));
	pAd->fgDumpStart = 1;
	...
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("idx: %d\n", idx));
	...
	do {
			...
			if (log_cb->entry[idx].un_dumped) {
				...
				if (!result->byPass) {
					MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("[RDD]0x%08x %08x\n", result->u4Prefix, result->u4Count));
	...
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("[After RDD dumping] idx: %d, end: %d\n", idx, end));

	/* RDD DUMP END */
	/* Count = Total number of 4 bytes RDD values divided by 2 */
	count_total = count_total / 2;
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("[HQA_GetDumpRDD]: Total Count As a Group of Two Pulses = %d\n", count_total));
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("[HQA_GetDumpRDD] idx = %d, log_cb->idx = %d, pAd->fgDumpStart = %d\n", idx, log_cb->idx, pAd->fgDumpStart));
	if ((idx == log_cb->idx) && (pAd->fgDumpStart)) {
		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("[RDD DUMP END]\n"));
		pAd->fgDumpStart = 0;
	}
	OS_SPIN_LOCK(&log_cb->lock);
	log_cb->is_dumping = FALSE;
...
#define MAX_DUMP_SIZE_OUT_RXV 40
	...
	ATECtrl->control_band_idx = (UCHAR)band_idx;
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			("%s: band_idx:%x, en_log:%x\n", __func__, band_idx, ATECtrl->en_log));
...
static INT32 hqa_start_tx_ext(PRTMP_ADAPTER pAd, RTMP_IOCTL_INPUT_STRUCT *wrq, struct _HQA_CMD_FRAME *cmd_frame)
{
	...
	if (band_idx == TESTMODE_BAND0)
			...
			/* force band1 as wcid 1 */
			param.wlan_id = 1;
		       TESTMODE_SET_PARAM(ate_ctrl, 1, wcid_ref, param.wlan_id);
	}
#else
	ate_ctrl->wcid_ref = param.wlan_id;
#endif
	...
	struct _HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	/*
	 * for cmd not supported by driver, always response to QATool to avoid
	 * QATool timeout/retry which will lead to GUI hang for a while.
	 * for cmd supported by driver, response is done in cmd handler.
	 */
	if (!bFindHandler)
		ResponseToQA(HqaCmdFrame, Wrq, 2, Status);
