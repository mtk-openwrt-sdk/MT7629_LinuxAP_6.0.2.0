...
#if defined(MT7626)
	wdev_idx = TESTMODE_GET_PARAM(ATECtrl, ATECtrl->control_band_idx, wdev_idx);
	wdev = pAd->wdev_list[wdev_idx];
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			 ("%s: Sa = %s,ATECtrl->wdev_idx=%d , wdev_idx=%d\n", __func__, Arg, ATECtrl->wdev_idx, wdev_idx));
#else
...
INT ATEManualParsingParam(RTMP_ADAPTER *pAd, RTMP_STRING *type, RTMP_STRING *val)
{
	...
	if (strcmp("ownmac", type)  == 0) {
		...
		if (ATE_ON(pAd)) {
			...
			wdev = pAd->wdev_list[wdev_idx];
			pAd->AteManualConnInfo.ownmac_idx = HcGetOmacIdx(pAd, wdev);
			own_mac_idx = pAd->AteManualConnInfo.ownmac_idx;
		} else {
	...
	RTMP_STRING *Arg)
{
	...
	if (ATE_ON(pAd)) {
		...
		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\x1b[42m%s: wdev_idx = %d\x1b[m\n", __func__, wdev_idx));
		...
		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\x1b[42m%s: OwnMacIdx = %d\x1b[m\n", __func__, OwnMacIdx));
	}
#endif/* #if defined(MT7626) */
	...
	RTMP_STRING *Arg)
{
	...
	if (!strcmp(Arg, "ATESTART") && (mode != ATE_START)) { /* support restart w/o ATESTOP */
		...
		if (ATEOp->DBDCTxTone)
			ATEOp->DBDCTxTone(pAd, 1, ant, ToneType, ToneFreq, DcOffset_I, DcOffset_Q, Band);
#else
		INT32 pwr1 = 0xf;
		INT32 pwr2 = 0;
