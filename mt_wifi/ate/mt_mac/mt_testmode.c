	...
	for (control_band_idx = TESTMODE_BAND0; control_band_idx <= TESTMODE_BAND1; control_band_idx++) {
		...
		if (regE3C & RO_AD_WF_SX_FCAL_INC_UP || regE3C & RO_AD_WF_SX_FCAL_INC_DN) {
			...
			for (i = 0; i < 10; i++) {
				...
				MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_INFO,
						("%s(%d), toggle #%d, WfSel:%d, regC:0x%x\n", __func__, __LINE__, i, WfSel, regC));
				ATEOp->RfRegWrite(pAd, WfSel, 0xc, regC);
				regC &= ~BIT(16);
	...
	BOOLEAN bRwFlag)
{
	...
	if (!IsFound) {
		...
		if (bRwFlag == ATE_WRITE_FLAG)
			HW_IO_WRITE32(pAd->hdev_ctrl, 0x90000 + RemapOffset, *upCrValue);
		else
			HW_IO_READ32(pAd->hdev_ctrl, 0x90000 + RemapOffset, upCrValue);
		HW_IO_WRITE32(pAd->hdev_ctrl, 0x700c, RestoreValue);
	} else {
		if (bRwFlag == ATE_WRITE_FLAG)
			HW_IO_WRITE32(pAd->hdev_ctrl, Offset, *upCrValue);
		else
			HW_IO_READ32(pAd->hdev_ctrl, Offset, upCrValue);
	}

}

static VOID MT_ATETxAcqStatus(RTMP_ADAPTER *pAd, UINT32 band_idx)
{
	UINT32 uTxQueueNum, arb_cr_val = 0;
	UINT32 uTqSW, uTqPW;
	UINT8 uTxQ_IDX;

	if (band_idx == TESTMODE_BAND0) {
		uTxQ_IDX = TxQ_IDX_AC1;
		uTxQueueNum = uTxQ_IDX << 24 | uTxQ_IDX << 16 | uTxQ_IDX << 8 | uTxQ_IDX;
		uTqSW = ARB_TQSW0;
		uTqPW = ARB_TQPW0;
	} else {
		uTxQ_IDX = TxQ_IDX_AC11;
		uTxQueueNum = uTxQ_IDX << 24 | uTxQ_IDX << 16 | uTxQ_IDX << 8 | uTxQ_IDX;
		uTqSW = ARB_TQSW1;
		uTqPW = ARB_TQPW1;
	}
	...
	/* enable debug flag */
	arb_cr_val = 0xf;
	MT_ATEIoRegRW(pAd, 0x820f082C, &arb_cr_val, ATE_WRITE_FLAG);
	/* enable debug clock */
	arb_cr_val = 0x1f;
	MT_ATEIoRegRW(pAd, 0x80025100, &arb_cr_val, ATE_WRITE_FLAG);
	/* arb flag */
	arb_cr_val = 0x04040404;
	MT_ATEIoRegRW(pAd, 0x80025104, &arb_cr_val, ATE_WRITE_FLAG);
	/* queue number */
	MT_ATEIoRegRW(pAd, 0x820f3060, &uTxQueueNum, ATE_WRITE_FLAG);
	/* debug flag */
	arb_cr_val = 0x04040808;
	MT_ATEIoRegRW(pAd, 0x80025108, &arb_cr_val, ATE_WRITE_FLAG);
	/* cr value */
	arb_cr_val = 0;
	MT_ATEIoRegRW(pAd, 0x820f0024, &arb_cr_val, ATE_READ_FLAG);

	if ((arb_cr_val & 0xF000) == 0 && (arb_cr_val & 0x700) == 0x100 &&
		(arb_cr_val & 0xFF) == 0) {

		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF,
				("\033[31m%s:: running band:%u, reset txq:%u, arb_cr_val:%x\033[0m\n",
				__func__, band_idx, uTxQ_IDX, arb_cr_val));

		/* Reset txq */
		/* stop tx queue */
		arb_cr_val = 0x1f00;
		MT_ATEIoRegRW(pAd, uTqPW, &arb_cr_val, ATE_WRITE_FLAG);
		/* enable tx queue */
		arb_cr_val = 0x1f1f1f1f;
		MT_ATEIoRegRW(pAd, uTqSW, &arb_cr_val, ATE_WRITE_FLAG);
	}
}
#endif /* ifdef MT7626 */
...
static INT32 TESTMODE_PeriodicThread_INIT(RTMP_ADAPTER *pAd)
{
	...
	if (!p_cb->is_init) {
		...
		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			("%s: Initialize Periodic thread Fail\n", __func__));
	}

	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		("%s: Initialize Periodic thread OK\n", __func__));

	return Ret;
err0:
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
		("%s: tx thread Periodic create fail\n", __func__));
	return Ret;
}
...
static INT TestMode_PeriodicThread(ULONG Context)
{
	...
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s: Periodic Thread running \n", __func__));
	RTMP_OS_COMPLETE(&ate_ctrl->cmd_done);
...
err1:
	...
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_INFO,
			("%s: leave, service_stat:%d\n", __func__, p_cb->service_stat));
	RTMP_CLEAR_FLAG(pAd, fRTMP_ADAPTER_HALT_IN_PROGRESS);
	return ret;
}
#endif /* MT7626 */
...
INT MT_ATERxDoneHandle(RTMP_ADAPTER *pAd, RX_BLK *pRxBlk)
{
...

	{
		...
		case RMAC_RX_PKT_TYPE_RX_NORMAL:
			...
			MAC_IO_READ32(pAd->hdev_ctrl, RMAC_CHFREQ1, &chfreq1);
...
#if defined(MT7626)
			INC_COUNTER64(pAd->WlanCounters[TESTMODE_BAND0].ReceivedFragmentCount);
#endif /* defined(MT7626) */
		}
...
#if defined(MT7626)
			INC_COUNTER64(pAd->WlanCounters[TESTMODE_BAND1].ReceivedFragmentCount);
#endif /* defined(MT7626) */
		}
#endif
#endif /* MT7615 */
...
INT8 mt_ate_init_wdev(RTMP_ADAPTER *pAd, UINT32 band_idx)
{
...
#if defined(MT7626)
	{
		...
		INT map_idx = 0;
		if (2 == wlan_config_get_tx_stream(pWdev)) {
...
#ifdef ATE_TXTHREAD
#if defined(MT7626)
	RtmpOsTaskWakeUp(&pAd->ATECtrl.periodic_thread.task);
#endif
#endif
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		("%s: wdev_idx:%u\n", __func__, ATECtrl->wdev_idx));
...
#if defined(DBDC_MODE) && defined(ATE_CONCURRENT_SUPPORT)
#if defined(MT7626)
	/* wait packet delivery to hw */
	msleep(10);
	MT_ATETxAcqStatus(pAd, control_band_idx);
#endif /* defined (MT7626) */
#endif
					...
					UINT32 Ch_Band)
{
	...
	mt7626_apply_dpd_flatness_data(pAd, ch_cfg);
#endif
#endif
...
#if defined(MT7626)
		/* avoid SW mgme's queue full which causes tx thread to stop */
