...
INT GroupPreCalInfoAlloc_7663(RTMP_ADAPTER *pAd, VOID **pptr, UINT16 idx, UINT32 length)
{
	UINT32 i = 0;
	P_PRE_CAL_INFO_MT7663_T pGroupPreCalInfo;

	if (os_alloc_mem(pAd, (UCHAR **)&pGroupPreCalInfo, sizeof(PRE_CAL_INFO_MT7663_T)) != NDIS_STATUS_SUCCESS)
		return 0;

	*pptr = pGroupPreCalInfo;
	os_zero_mem(pGroupPreCalInfo, sizeof(PRE_CAL_INFO_MT7663_T));
	...
	return sizeof(PRE_CAL_INFO_MT7663_T);
}

INT DpdFlatnessCalInfoAlloc_7663(RTMP_ADAPTER *pAd, VOID **pptr, UINT16 idx, UINT32 length)
{
	UINT32 i = 0;
	P_PRE_CAL_INFO_MT7663_T pDpdFlatnessCalInfo;

	if (os_alloc_mem(pAd, (UCHAR **)&pDpdFlatnessCalInfo, sizeof(PRE_CAL_INFO_MT7663_T)) != NDIS_STATUS_SUCCESS)
		return 0;

	*pptr = pDpdFlatnessCalInfo;
	os_zero_mem(pDpdFlatnessCalInfo, sizeof(PRE_CAL_INFO_MT7663_T));
...
#else
	os_move_mem(pDpdFlatnessCalInfo->au4Data, pAd->TxDPDImage + (idx * DPD_FLATNESS_CAL_SIZE), length);
#endif
	return sizeof(PRE_CAL_INFO_MT7663_T);
}
