...
#ifdef MT7626
INT32 MtCmdCalCtrl(RTMP_ADAPTER *pAd, INT32 BandIdx, INT32 CalFuncIdx, UINT32 CalItemIdx)
{
	struct cmd_msg *msg;
	CAL_CTRL_FUNC_T calCtrl;
	INT32 ret = 0;
	struct _CMD_ATTRIBUTE attr = {0};
	MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_INFO,
			("%s: BandIdx = %d, CalFuncIdx = %d, CalItemIdx = %x\n",
			__func__, BandIdx, CalFuncIdx, CalItemIdx));
	msg = MtAndesAllocCmdMsg(pAd, sizeof(calCtrl));
	...
	SET_CMD_ATTR_MCU_DEST(attr, HOST2N9);
	SET_CMD_ATTR_TYPE(attr, EXT_CID);
	SET_CMD_ATTR_EXT_TYPE(attr, EXT_CMD_ID_CAL_CTRL);
	SET_CMD_ATTR_CTRL_FLAGS(attr, INIT_CMD_SET);
	SET_CMD_ATTR_RSP_WAIT_MS_TIME(attr, 0);
	SET_CMD_ATTR_RSP_EXPECT_SIZE(attr, 0);
	SET_CMD_ATTR_RSP_WB_BUF_IN_CALBK(attr, NULL);
	SET_CMD_ATTR_RSP_HANDLER(attr, NULL);
	MtAndesInitCmdMsg(msg, attr);
	os_zero_mem(&calCtrl, sizeof(calCtrl));
	calCtrl.u1DbdcIdx = (UINT8)BandIdx;
	calCtrl.u1FuncIdx = (UINT8)CalFuncIdx;
	calCtrl.u4CalIdx = CalItemIdx;
	MtAndesAppendCmdMsg(msg, (char *)&calCtrl, sizeof(calCtrl));
	ret = chip_cmd_tx(pAd, msg);
error:
	MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_INFO,
			 ("%s:(ret = %d)\n", __func__, ret));
