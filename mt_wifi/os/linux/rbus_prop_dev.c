...
#ifdef MT7626
	{PCI_DEVICE(0x14c3, 0x7626)},
	{PCI_DEVICE(0x14c3, 0x7629)},
#endif /* MT7626 */
	{} /* terminate list */
};
...
static int DEVINIT mt_rbus_probe(struct pci_dev *pdev, const struct pci_device_id *pci_id)
{
...
#if defined(MT7626)
	{
		...
		extern unsigned int dev_second_irq;

		if ((int)dev_second_irq < 0) {
			MTWF_LOG(DBG_CAT_INIT, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
					 ("%s(): MD int irq num (%d) does not exist\n", __func__, dev_second_irq));
		} else {
			pAd->dev_irq_md = dev_second_irq; /* <GIC_SPI 129 IRQ_TYPE_LEVEL_LOW> */
		}
