...
BOOLEAN CFG80211DRV_OpsLeave(VOID *pAdOrg, PNET_DEV pNetDev)
{
	...
	pAd->StaCfg[0].MlmeAux.AutoReconnectSsidLen = 32;
	os_zero_mem(pAd->StaCfg[0].MlmeAux.AutoReconnectSsid, pAd->StaCfg[0].MlmeAux.AutoReconnectSsidLen);
	os_alloc_mem(pAd, (UCHAR **)&pMsgElem, sizeof(MLME_QUEUE_ELEM));
#if defined(RT_CFG80211_P2P_CONCURRENT_DEVICE) || defined(CFG80211_MULTI_STA)

	if (isVifDevice)
		COPY_MAC_ADDR(DeAuthReq.Addr, pAd->StaCfg[MAIN_MBSSID].MlmeAux.Bssid);
	else
#endif /* RT_CFG80211_P2P_CONCURRENT_DEVICE || CFG80211_MULTI_STA */
		COPY_MAC_ADDR(DeAuthReq.Addr, pAd->StaCfg[0].Bssid);
	...
	pMsgElem->wdev = &(pAd->StaCfg[0].wdev);
...
#endif /* RT_CFG80211_P2P_CONCURRENT_DEVICE || CFG80211_MULTI_STA */
		LinkDown(pAd, FALSE, &(pAd->StaCfg[0].wdev));
