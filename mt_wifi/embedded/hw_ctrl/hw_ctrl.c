...
static NTSTATUS HwCtrlUpdateProtect(RTMP_ADAPTER *pAd)
{
...
#ifdef DBDC_MODE
	MT_PROTECT_CTRL_T  protect_band1;
#endif /* DBDC_MODE */
	os_zero_mem(&protect, sizeof(MT_PROTECT_CTRL_T));
#ifdef DBDC_MODE
	os_zero_mem(&protect_band1, sizeof(MT_PROTECT_CTRL_T));
#endif /* DBDC_MODE */
	...
	do {
		...
		if ((pAd->CommonCfg.dbdc_mode == TRUE) && (HcGetBandByWdev(wdev) == DBDC_BAND1)) {
			/*
			   Some DBDC CHIP ex : PHY1 of MT7626 didn't support CCK Rate,
			   (OMAC2 apclix0 can't set ERP On!, it will lead the CCK Rate CTS to self, and LMAC hang! )
			*/
			if (mode & SET_PROTECT(ERP)) {
				if (WMODE_CAP_2G(wdev->PhyMode)) {
					protect_band1.erp_mask |= ERP_OMAC0;
					if ((wdev->wdev_type == WDEV_TYPE_STA))
						protect_band1.erp_mask |= (1 << wdev->OmacIdx);
				}
			}
			if (mode & SET_PROTECT(NON_MEMBER_PROTECT)) {
				protect_band1.mix_mode = 1;
				protect_band1.gf = 1;
				protect_band1.bw40 = 1;
			}
			...
			protect_band1.band_idx = DBDC_BAND1;
		} else
#endif /* DBDC_MODE */
		{
			/*
			...
			*/
			if (mode & SET_PROTECT(ERP)) {
				if (WMODE_CAP_2G(wdev->PhyMode)) {
					protect.erp_mask |= ERP_OMAC0;
					if ((wdev->wdev_type == WDEV_TYPE_STA))
						protect.erp_mask |= (1 << wdev->OmacIdx);
				}
			}
			if (mode & SET_PROTECT(NON_MEMBER_PROTECT)) {
				protect.mix_mode = 1;
	...
	if (pAd->CommonCfg.dbdc_mode == TRUE)
		arch_ops->archUpdateProtect(pAd, &protect_band1);
...
static INT ErrRecoveryMcuIntEvent(RTMP_ADAPTER *pAd, UINT32 status)
{
	...
	if (IS_MT7663(pAd) || IS_AXE(pAd) || IS_MT7626(pAd))
		u4McuInitEvent = MT7663_MCU_INT_EVENT;
#endif
...
static NTSTATUS HwCtrlPwrMgtBitWifi(RTMP_ADAPTER *pAd, HwCmdQElmt *CMDQelmt)
{
	...
	AsicExtPwrMgtBitWifi(pAd, rpMtPwrMgtBitWifi->ucWlanIdx, rpMtPwrMgtBitWifi->ucPwrMgtBit);
	return NDIS_STATUS_SUCCESS;
}
