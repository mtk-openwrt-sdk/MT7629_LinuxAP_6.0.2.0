...
VOID ap_peer_auth_req_at_idle_action(RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM *Elem)
{
	...
	checkAuthSanity = ap_peer_auth_sanity(pAd, Elem->Msg, Elem->MsgLen, &auth_info);
...
#define	RADIUS_ACL_ACCEPT_TIMEOUT  3
	...
	if ((auth_info.auth_alg == AUTH_MODE_SAE) &&
		...
		if ((auth_info.auth_alg == AUTH_MODE_OPEN) &&
			...
			/* wrong algorithm */
			ap_peer_auth_simple_rsp_gen_and_send(pAd, pRcvHdr, auth_info.auth_alg, auth_info.auth_seq + 1, MLME_ALG_NOT_SUPPORT);
