...
#if defined(MT7626)
#define UMAC_BSS_GROUP_NUMBER               4
#else
#define UMAC_BSS_GROUP_NUMBER               16
#endif
#define UMAC_WLAN_ID_MAX_VALUE          (MAX_LEN_OF_MAC_TABLE-1)
...
INT vow_set_rx_airtime(PRTMP_ADAPTER pad, UINT8 cmd, UINT32 subcmd)
{
	...
	case ENUM_RX_AT_FEATURE_CTRL:
		...
		case ENUM_RX_AT_FEATURE_SUB_TYPE_WTBL_ADM_EN:
			rx_at_ctrl.rRxAtGeneralCtrl.rRxAtFeatureSubCtrl.fgRxWtblAdmTimeEn = pad->vow_rx_time_cfg.rx_time_wtbl_adm_en;
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s:(cmd = 0x%x, subcmd = 0x%x, value = 0x%x)\n",
					 __func__, cmd, subcmd, pad->vow_rx_time_cfg.rx_time_wtbl_adm_en));
			break;

		case ENUM_RX_AT_FEATURE_SUB_TYPE_WTBL_ADM_MODE:
			rx_at_ctrl.rRxAtGeneralCtrl.rRxAtFeatureSubCtrl.ucRxWtblAdmTimeMode = pad->vow_rx_time_cfg.wtbl_adm_mode;
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s:(cmd = 0x%x, subcmd = 0x%x, value = 0x%x)\n",
					 __func__, cmd, subcmd, pad->vow_rx_time_cfg.wtbl_adm_mode));
			break;
...
void vow_variable_reset(PRTMP_ADAPTER pAd)
{
	...
	pAd->vow_rx_time_cfg.rx_time_wtbl_adm_en = FALSE;
	pAd->vow_rx_time_cfg.wtbl_adm_mode = VOW_WTBL_ADM_CDT_CRT;
...
VOID vow_init_rx(PRTMP_ADAPTER pad)
{
	...
	if (vow_is_enabled(pad) == FALSE) {
		...
		/* disable rx adm */
		pad->vow_rx_time_cfg.rx_time_wtbl_adm_en = FALSE;
		vow_set_rx_airtime(pad, ENUM_RX_AT_FEATURE_CTRL, ENUM_RX_AT_FEATURE_SUB_TYPE_WTBL_ADM_EN);
		/* switch adm format */
		pad->vow_rx_time_cfg.wtbl_adm_mode = VOW_WTBL_ADM_CTT_CTB;
		vow_set_rx_airtime(pad, ENUM_RX_AT_FEATURE_CTRL, ENUM_RX_AT_FEATURE_SUB_TYPE_WTBL_ADM_MODE);
		return;
	}
	...
	ret = vow_set_rx_airtime(pad, ENUM_RX_AT_FEATURE_CTRL, ENUM_RX_AT_FEATURE_SUB_TYPE_WTBL_ADM_EN);
	/* set adm format */
#ifdef RED_SUPPORT
	if (pad->red_en)
		pad->vow_rx_time_cfg.wtbl_adm_mode = VOW_WTBL_ADM_CDT_CRT;
	...
	pad->vow_rx_time_cfg.wtbl_adm_mode = VOW_WTBL_ADM_CTT_CTB;
#endif
	ret = vow_set_rx_airtime(pad, ENUM_RX_AT_FEATURE_CTRL, ENUM_RX_AT_FEATURE_SUB_TYPE_WTBL_ADM_MODE);
	/* set ED offset */
	...
	IN  RTMP_STRING * arg)
{
	...
	if (arg) {
		...
		if ((rv > 0)) {
			...
			pAd->vow_rx_time_cfg.wtbl_adm_mode = val;
			ret = vow_set_rx_airtime(pAd, ENUM_RX_AT_FEATURE_CTRL, ENUM_RX_AT_FEATURE_SUB_TYPE_WTBL_ADM_MODE);
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
					 ("%s: set %u.\n", __func__, val));
...
INT set_vow_rx_airtime_show_en(
	IN  PRTMP_ADAPTER pAd,
	IN  RTMP_STRING * arg)
{
	UINT32 val, rv;
	...
	if (arg) {
		...
		if ((rv > 0)) {
			...
			pAd->vow_rx_time_cfg.rx_time_wtbl_adm_en = val;
			ret = vow_set_rx_airtime(pAd, ENUM_RX_AT_FEATURE_CTRL, ENUM_RX_AT_FEATURE_SUB_TYPE_WTBL_ADM_EN);
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
					 ("%s: set %u.\n", __func__, val));
	...
	IN  RTMP_STRING * arg)
{
	...
	if (arg) {
		...
		if ((rv > 0) && (group < VOW_MAX_GROUP_NUM)) {
			...
			val |= group;
			RTMP_IO_WRITE32(pAd->hdev_ctrl, 0x8388, val);
			RTMP_IO_READ32(pAd->hdev_ctrl, 0x8358, &val1_ori);
			RTMP_IO_READ32(pAd->hdev_ctrl, 0x835c, &val2_ori);
	...
	IN  RTMP_STRING * arg)
{
	...
	if (arg) {
		...
		if ((rv > 0) && (sta < MAX_LEN_OF_MAC_TABLE)) {
			...
			/* BK */
			RTMP_IO_WRITE32(pAd->hdev_ctrl, 0x8388, val);
			RTMP_IO_READ32(pAd->hdev_ctrl, 0x8358, &val1);
			/* BE */
			val |= 1;
			RTMP_IO_WRITE32(pAd->hdev_ctrl, 0x8388, val);
			RTMP_IO_READ32(pAd->hdev_ctrl, 0x8358, &val2);
			/* VI */
			val |= 2;
			RTMP_IO_WRITE32(pAd->hdev_ctrl, 0x8388, val);
			RTMP_IO_READ32(pAd->hdev_ctrl, 0x8358, &val3);
			...
			val |= 3;
			RTMP_IO_WRITE32(pAd->hdev_ctrl, 0x8388, val);
			RTMP_IO_READ32(pAd->hdev_ctrl, 0x8358, &val4);
	...
	IN  PRTMP_ADAPTER pAd)
{
	if (pAd->vow_show_en == 1) {
		CHAR i, ac, bw;
		UINT32 wtbl_offset;
		UINT32 tx_sum, tx, rx_sum, rx, tx_ok[2], tx_fail[2], tx_cnt, tx_ok_sum,
			tx_fail_sum, tx_diff_time, rx_diff_time;
		UINT32 cnt, free_cnt;
		UINT32 ple_stat[33] = {0}, pg_flow_ctrl[6] = {0};
		UINT32 sta_pause[8] = {0}, dis_sta_map[8] = {0};
		INT32 k, l;
		UINT32 counter[2];
...
#if defined(MT7626)
		{
			static UINT32 vow_ampdu_cnt_backup[DBDC_BAND_NUM][1];
			UINT32 band_idx = 0;
			COUNTER_802_11 *wlanCounter;

			for (band_idx = 0; band_idx < DBDC_BAND_NUM; band_idx++) {
				wlanCounter = &pAd->WlanCounters[band_idx];
				if (vow_ampdu_cnt_backup[band_idx][0] > (wlanCounter->AmpduSuccessCount.u.LowPart + wlanCounter->AmpduFailCount.u.LowPart))
					vow_ampdu_cnt_backup[band_idx][0] = 0;
				vow_ampdu_cnt = (vow_ampdu_cnt +
								(wlanCounter->AmpduSuccessCount.u.LowPart + wlanCounter->AmpduFailCount.u.LowPart) -
								vow_ampdu_cnt_backup[band_idx][0]);
				vow_ampdu_cnt_backup[band_idx][0] = (wlanCounter->AmpduSuccessCount.u.LowPart + wlanCounter->AmpduFailCount.u.LowPart) ;
			}
		}
#else
		RTMP_IO_READ32(pAd->hdev_ctrl, MIB_M0SDR14, &cnt);
		vow_ampdu_cnt += cnt;
...
{
	UINT32 reg32;
	HW_IO_READ32(pad->hdev_ctrl, WTBL_OFF_ACR, &reg32);
	/* disable RX airtime */
	reg32 &= ~WTBL_ADM_RX_EN;
	/* switch adm format to 0 to avoid wtbl hang HW issue by ADM counter overflow */
	reg32 &= ~(WTBL_ADM_FORMAT_B0 | WTBL_ADM_FORMAT_B1);
	HW_IO_WRITE32(pad->hdev_ctrl, WTBL_OFF_ACR, reg32);
	HW_IO_READ32(pad->hdev_ctrl, AIRTIME_DRR_SIZE, &reg32);
	reg32 &= ~STA_MAX_DEFICIT_MASK;
	reg32 |= 1; /* 256us */
	HW_IO_WRITE32(pad->hdev_ctrl, AIRTIME_DRR_SIZE, reg32);
	/* disable RTS failed airtime charge for RTS deadlock */
	HW_IO_READ32(pad->hdev_ctrl, AGG_SCR, &reg32);
	reg32 |= RTS_FAIL_CHARGE_DIS;
	HW_IO_WRITE32(pad->hdev_ctrl, AGG_SCR, reg32);

	/* for Gen_2/TALOS, set MAX_BFF_TIME to 10ms*/
	if ((pad->vow_gen.VOW_GEN == VOW_GEN_2) ||
	    (pad->vow_gen.VOW_GEN == VOW_GEN_TALOS)) {
		HW_IO_READ32(pad->hdev_ctrl, AGG_ATCR0, &reg32);
		reg32 |= MAX_BFF_TIME_EN;
		reg32 |= MAX_BFF_TIME;
		HW_IO_WRITE32(pad->hdev_ctrl, AGG_ATCR0, reg32);
	}
}
#endif /* VOW_SUPPORT */
