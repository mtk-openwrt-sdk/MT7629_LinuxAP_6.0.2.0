	...
	PEER_PROBE_REQ_PARAM * ProbeReqParam)
{
	...
	if (FrameType == SYNC_FSM_PEER_PROBE_REQ) {
	...
	} else if (FrameType == AUTH_FSM_PEER_AUTH_REQ) {
...
/* WPS_BandSteering Support */
	...
	if (entry && (FrameType == AUTH_FSM_PEER_AUTH_REQ) && (entry->BndStrg_Sta_State == BNDSTRG_STA_ASSOC)) {
	...
	if (entry) {
		...
		FrameType == SYNC_FSM_PEER_PROBE_REQ ? ("probe") : (FrameType == 3 ? "auth" : "unknow"), PRINT_MAC(pSrcAddr)));
#endif
		return TRUE;
	} else {
#ifdef BND_STRG_QA
