	...
	IN RTMP_STRING * arg)
{
	...
	MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("BSS\tMAC\t\t\tAID\tWCID\tAuthMode\tPairwiseCipher\tPortSecured\n"));
	...
	for (idx = 0; idx < MAX_LEN_OF_MAC_TABLE; idx++) {
		...
		if (pEntry && IS_ENTRY_CLIENT(pEntry) && pEntry->Sst == SST_ASSOC) {
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%d\t%02X:%02X:%02X:%02X:%02X:%02X\t%d\t%d\t%s\t\t%s\t\t%s\n",
					 tr_entry->wdev->func_idx,
