	...
	if (pAutoChCtrl->pChannelInfo != NULL) {
		...
		*/
		...
		if ((pAd->ScanCtrl[BandIdx].ScanTime[pAd->ScanCtrl[BandIdx].CurrentGivenChan_Index]) != 0) {
			...
			asic_calculate_nf(pAd, pAd->ChannelInfo.bandidx);
...
#define CCA_THRESHOLD (100)
	...
	if (pChannelInfo == NULL) {
		MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("pChannelInfo equal NULL.\n"));
		return FirstChannel(pAd, wdev);
	}
	...
	IN struct wifi_dev *wdev)
{
		...
		for (channel_idx = 0; channel_idx < ch_ctrl->ChListNum; channel_idx++) {
			...
			if (cfg_ht_bw == BW_20) {
				...
				if (vht80_channel_group(pAd, ch)) {
					auto_ch_ctrl->pChannelInfo->supp_ch_list[ch_list_num].channel = ch_ctrl->ChList[channel_idx].Channel;
					auto_ch_ctrl->pChannelInfo->supp_ch_list[ch_list_num].cen_channel = vht_cent_ch_freq (ch, VHT_BW_80);
					auto_ch_ctrl->pChannelInfo->supp_ch_list[ch_list_num].dfs_req = ch_ctrl->ChList[channel_idx].DfsReq;
					if (auto_ch_ctrl->pChannelInfo->SkipList[channel_idx] == TRUE)
						auto_ch_ctrl->pChannelInfo->supp_ch_list[ch_list_num].skip_channel = TRUE;
					ch_list_num++;
	...
	for (channel_idx = 0; channel_idx < auto_ch_ctrl->pChannelInfo->channel_list_num; channel_idx++) {
		MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("[%s] Support channel: PrimCh=%d, CentCh=%d, DFS=%d, skip %d\n",
			__FUNCTION__, auto_ch_ctrl->pChannelInfo->supp_ch_list[channel_idx].channel, auto_ch_ctrl->pChannelInfo->supp_ch_list[channel_idx].cen_channel,
	...
	IN struct wifi_dev *wdev)
{
	...
	MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s Insert new group channel list Number=%d cen_channel=%d"
		" best_ctrl_channel=%d BUSY_TIEM=%d, skip_group=%d, grp_score=%d\n",
		__FUNCTION__, auto_ch_ctrl->pChannelInfo->group_ch_list_num, group_ch_list->cen_channel,
	...
	IN struct wifi_dev *wdev)
{
	...
	MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s Update group channel list index=%d cen_channel=%d "
		"best_ctrl_channel=%d BUSY_TIEM=%d skip_group=%d, grp_score=%d\n",
		__FUNCTION__, auto_ch_ctrl->pChannelInfo->group_ch_list_num, group_ch_list->cen_channel,
		group_ch_list->best_ctrl_channel, group_ch_list->max_busy_time, group_ch_list->skip_group, group_ch_list->grp_score));
}
	...
	IN struct wifi_dev *wdev)
{
	...
	for (i = 0; i < auto_ch_ctrl->pChannelInfo->group_ch_list_num; i++) {
		...
		if (auto_ch_ctrl->pChannelInfo->group_ch_list[i].skip_group == FALSE) {
			...
			MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("ChIdx=%d control-Channle=%d cen-channel=%d Max_BUSY_TIME=%d\n",
			...
			if ((busy <= min_busy) && (acs_grp_ch_list->bw80_not_allowed == FALSE) && (acs_grp_ch_list->bw40_not_allowed == FALSE)) {
				min_busy = busy;
						...
						for (l = -3; l < 4; l++) {
							...
							if (auto_ch_ctrl->pChannelInfo->channel_score[j+l].channel == auto_ch_ctrl->pChannelInfo->group_ch_list[i].bw80_grp_ch_member[k].channel) {
								...
								if ((auto_ch_ctrl->pChannelInfo->group_ch_list[i].bw80_not_allowed == TRUE) ||
									...
									MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Channel %d don't support BW80, force to assign min score(%d)!!!!!!\n",
						...
						for (l = -1; l < 2; l++) {
							...
							if (auto_ch_ctrl->pChannelInfo->channel_score[j+l].channel == auto_ch_ctrl->pChannelInfo->group_ch_list[i].bw40_grp_ch_member[k].channel) {
								...
								if ((auto_ch_ctrl->pChannelInfo->group_ch_list[i].bw40_not_allowed == TRUE) ||
									...
									auto_ch_ctrl->pChannelInfo->channel_score[j+l].score = min_score;
									MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Channel %d don't support BW40, force to assign min score(%d)!!!!!!\n",
										auto_ch_ctrl->pChannelInfo->channel_score[j+l].channel, auto_ch_ctrl->pChannelInfo->channel_score[j+l].score));
								}
								if (auto_ch_ctrl->pChannelInfo->channel_score[j+l].score > 100)
									auto_ch_ctrl->pChannelInfo->channel_score[j+l].score = 100;
	...
	for (j = 0; j < ch_ctrl->ChListNum; j++) {
		...
		MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("CH%d final score is %d\n", auto_ch_ctrl->pChannelInfo->channel_score[j].channel,
			 auto_ch_ctrl->pChannelInfo->channel_score[j].score));
	}

	MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Min Busy Time=%d,select best channel %d\n", min_busy, best_channel));
	MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("====================================================\n"));
	...
	IN struct wifi_dev *wdev)
{
	...
	MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("=====================================================\n"));
	for (i = 0; i < auto_ch_ctrl->pChannelInfo->channel_list_num; i++) {
		MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Channel %d : Busy Time = %u, Score %d,Skip Channel = %s\n",
	...
	if (WMODE_CAP_5G(wdev->PhyMode)) {
		MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("=============== select best ch for 5G ===============\n"));
		acs_generate_group_channel_list(pAd, wdev);
		best_channel = find_best_channel_of_all_grp(pAd, wdev);
	} else
		MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("=============== select best ch for 2.4G =============\n"));
...
#ifdef DOT11_VHT_AC
	INT k, count, idx;
	struct vht_ch_layout *vht_ch_80M = get_ch_array(BW_80);
	struct vht_ch_layout *vht_ch_160M = get_ch_array(BW_160);
	...
	IN struct wifi_dev *pwdev)
{
	...
	MlmeEnqueueWithWdev(pAd, AUTO_CH_SEL_STATE_MACHINE,  AUTO_CH_SEL_SCAN_REQ, 0, NULL, pwdev->func_idx, pwdev, NULL);
	...
	IN PVOID SystemSpecific3)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s----------------->\n", __func__));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("[AutoChSelScanTimeout] - BandIdx = %d\n", BandIdx));
	MlmeEnqueueWithWdev(pAd, AUTO_CH_SEL_STATE_MACHINE,  AUTO_CH_SEL_SCAN_TIMEOUT, 0, NULL, 0, pwdev, NULL);
	RTMP_MLME_HANDLER(pAd);
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s<-----------------\n", __func__));
}
...
{
	UCHAR i, WdevBandIdx, BandIdx, HWBandNum;
