...
NDIS_STATUS update_mtb_value(struct _RTMP_ADAPTER *pAd, UCHAR profile_id, UINT_32 extra, RTMP_STRING *value)
{
	...
	case MTB_2G_PROFILE:
		...
		if (strcmp(value, mtb[dev_idx].profile_2g) && (str_len < L2PROFILE_PATH_LEN)) {
			strcpy(mtb[dev_idx].profile_2g, value);
			mtb[dev_idx].profile_2g[str_len] = '\0';
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
	...
	case MTB_5G_PROFILE:
		str_len = strlen(value);
		if (strcmp(value, mtb[dev_idx].profile_5g) && (str_len < L2PROFILE_PATH_LEN)) {
			strcpy(mtb[dev_idx].profile_5g, value);
			mtb[dev_idx].profile_5g[str_len] = '\0';
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
	...
	CHAR *final)
{
	...
	snprintf(tok_str, sizeof(tok_str), "%s%d", parm, start_idx);
...
{
	/*IEEE8021X*/
	multi_profile_merge_perband(mpf, "IEEE8021X", buf1, buf2, final);
	/*PreAuth*/
	multi_profile_merge_perband(mpf, "PreAuth", buf1, buf2, final);
	/*AuthMode*/
	multi_profile_merge_perband(mpf, "AuthMode", buf1, buf2, final);
	/*EncrypType*/
	multi_profile_merge_perband(mpf, "EncrypType", buf1, buf2, final);
	/*RekeyMethod*/
	multi_profile_merge_perband(mpf, "RekeyMethod", buf1, buf2, final);
	/*RekeyInterval*/
	multi_profile_merge_perband(mpf, "RekeyInterval", buf1, buf2, final);
	/*PMKCachePeriod*/
	multi_profile_merge_perband(mpf, "PMKCachePeriod", buf1, buf2, final);
	/*WPAPSK*/
	multi_profile_merge_increase(mpf, 1, "WPAPSK", buf1, buf2, final);
	/*DefaultKeyID*/
	multi_profile_merge_perband(mpf, "DefaultKeyID", buf1, buf2, final);
	/*KeyType & KeyStr*/
	...
	/*RADIUS_Server*/
	multi_profile_merge_perband(mpf, "RADIUS_Server", buf1, buf2, final);
	/*RADIUS_Port*/
	multi_profile_merge_perband(mpf, "RADIUS_Port", buf1, buf2, final);
	/*RADIUS_Key*/
	multi_profile_merge_perband(mpf, "RADIUS_Key", buf1, buf2, final);
	/*RADIUS Key%d*/
	multi_profile_merge_radius_key(mpf, 1, "RADIUS_Key", buf1, buf2, final);
	/*EAPifname*/
	multi_profile_merge_perband(mpf, "EAPifname", buf1, buf2, final);
	/*PreAuthifname*/
	multi_profile_merge_perband(mpf, "PreAuthifname", buf1, buf2, final);
	/*PMFMFPC*/
	multi_profile_merge_perband(mpf, "PMFMFPC", buf1, buf2, final);
	/*PMFMFPR*/
	multi_profile_merge_perband(mpf, "PMFMFPR", buf1, buf2, final);
	/*PMFSHA256*/
	multi_profile_merge_perband(mpf, "PMFSHA256", buf1, buf2, final);
	return NDIS_STATUS_SUCCESS;
}
	...
	CHAR *final)
{
	...
	multi_profile_merge_perband(data, "AutoChannelSelect", buf1, buf2, final);
#endif /* DBDC_MODE */
...
label_wsc_v2_done:
...
#ifdef WSC_AP_SUPPORT
	{
		...
		snprintf(value, sizeof(value), "%s;%s", WscConMode, WscConMode2);
		if (RTMPSetKeyParameter("WscConfMode", value, sizeof(value), final, TRUE) != TRUE)
			goto label_WscConfMode_done;
label_WscConfMode_done:
		...
		snprintf(value, sizeof(value), "%s;%s", WscConMode, WscConMode2);
		if (RTMPSetKeyParameter("WscConfStatus", value, sizeof(value), final, TRUE) != TRUE)
			goto label_WscConfStatus_done;
	}
...
{
	/*MboSupport*/
	multi_profile_merge_perband(data, "MboSupport", buf1, buf2, final);
	return NDIS_STATUS_SUCCESS;
}
#endif
