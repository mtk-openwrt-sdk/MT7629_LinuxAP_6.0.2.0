...
/*Reduced neighbor report IE should be final IE to be added, so we can determine the maximum length of Beacon*/
	...
	FrameLen += oce_build_ies(pAd, &vht_ie_info);
#endif
	...
	UCHAR BCN_UPDATE_REASON)
{
	if (wdev && !WDEV_WITH_BCN_ABILITY(wdev)) {
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s, Update reason = %x, devname:%s, caller=%pS\n",
			__func__, BCN_UPDATE_REASON, RtmpOsGetNetDevName(pAd->net_dev), OS_TRACE));
		goto end;
	}
...
#define PRE_BCN_CHECK_PERIOD 25 /* 2.5s */
	...
	if ((pAd->Mlme.PeriodicRound % PRE_BCN_CHECK_PERIOD) == 0) {
		...
		for (bandidx = 0; bandidx < HcGetAmountOfBand(pAd) ; bandidx++) {
			...
			MAC_IO_READ32(pAd->hdev_ctrl, MIB_M0SDR0 + band_offset, &mac_val);
